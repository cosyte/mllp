...
| Interoperability with Epic and Cerner interfaces | Unverified | Byte-level interoperability is proven against freely available engines only. Neither of those two is part of this package's harness, so their behaviour is inferred from the specification rather than observed. Validate against your actual peer before trusting a production interface. |
