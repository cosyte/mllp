...
- **Stable warning codes** are a public API. Renaming or removing one is a breaking change. Codes: `MLLP_MISSING_LEADING_VT`, `MLLP_FS_WITHOUT_CR`, `MLLP_LF_AFTER_FS`, `MLLP_LEADING_WHITESPACE`, `MLLP_TRAILING_BYTES`, `MLLP_PAYLOAD_CONTAINS_VT`, `MLLP_PAYLOAD_CONTAINS_FS`, `MLLP_EMPTY_PAYLOAD`, `MLLP_FRAME_TOO_LARGE`, `MLLP_ACK_UNMATCHED_CONTROL_ID`, `MLLP_ACK_AFTER_TIMEOUT`, `MLLP_ACK_INBOUND_UNPARSEABLE`, `MLLP_ACK_CONTROL_ID_NOT_VERBATIM` (13 total; the last **two** are `ack-from-hl7`-scoped — emitted in `MllpAck.warnings`, not through the framing registry).
- **The control ID is read ONCE, in one place** (MLLP-ACK-UTF8). `src/internal/control-id.ts` owns the byte-level MSH-10 / MSA-2 scanners (`latin1` decode, MSH-1 read dynamically from the segment's 4th byte per §2.5.4). Three call sites depend on agreeing byte-for-byte on what a control ID *is* — the client's correlator keys its in-flight store on it, `buildRawAck` echoes it into MSA-2, and `buildMllpAck` **verifies** its own output against it — and any disagreement between two of them is an ACK the sender cannot match: timeout → resend → **duplicate clinical message**. Each of those three re-derived it independently once, and each got it wrong differently (`ascii` masking, a hardcoded `|`, a `utf8` round-trip). Do not re-implement a fourth.
