...
- **Caller-supplied Diffie-Hellman parameters** (`plain-owls-supply.md`). Purely additive on the same
  ...
  NOT do: it checks the prime and the generator against the bounds the TLS library's own parameter
  check applies, which costs a comparison, and it does not test the prime for primality, which costs
  seconds on a 3072-bit group and is not what `listen()` is for. A well-formed block whose modulus is
  composite is therefore accepted, exactly as the library accepts it. Recorded rather than left to be
  inferred from the refusal path.
