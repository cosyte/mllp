...
- **CI gate.** The `format`/`format:check` globs now cover `scripts/**/*.mjs`, so the script is prettier-gated in CI (it was matched by no glob before). Widening the gate also reformatted the pre-existing `scripts/generate-test-certs.mjs` (cosmetic quote/wrap only).
