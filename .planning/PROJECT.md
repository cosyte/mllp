...
- [ ] Partial-read assembly across arbitrary TCP chunk boundaries _(FRAME-04..06)_
- [ ] Opt-in tolerance for common framing deviations (FS-only, FS+LF, missing leading VT, embedded VT/FS) with stable warning codes _(FRAME-07..10, WARN-01..05)_
- [ ] MLLP server: listen, accept, emit framed messages as `Buffer`, graceful shutdown _(SERVER-01..07)_
- [ ] MLLP client: connect, send, await ACK with timeout, auto-reconnect with exponential backoff, backpressure _(CLIENT-01..09)_
- [ ] Explicit, inspectable connection state machine (`CONNECTING` / `CONNECTED` / `DRAINING` / `DISCONNECTED`) with lifecycle events _(LIFE-01..05)_
- [ ] ACK helpers: build `AA` / `AE` / `AR` from a parsed inbound message, plus raw pass-through _(ACK-01..05)_
- [ ] TLS support for both client and server (including SNI, client certs) _(TLS-01..04)_
- [ ] In-memory transport adapter for deterministic, socket-free tests _(TRANS-01..04)_
- [ ] Typed errors (`MllpFramingError`, `MllpTimeoutError`, `MllpConnectionError`) with stable codes _(ERR-01..04)_
...
- [ ] Three runnable examples + comprehensive README _(DOCS-01..05)_
...
- **Runtime deps:** Zero. Node stdlib only (`net`, `tls`, `stream`, `events`, `buffer`, `timers`). Dev deps (Vitest, TypeScript, linters) fine.
...
| Explicit connection state machine, not socket flags | Node's `net.Socket` has half-open states, `writable`/`readable` flags that drift, and no built-in concept of "draining in-flight ACKs before close." We expose a 4-state FSM (`CONNECTING` / `CONNECTED` / `DRAINING` / `DISCONNECTED`) with transition events. | — Pending |
