 ...
 * On this suite's biggest payloads that is the whole cost of the test: timed
 * around the assertion alone on an identical 1 MiB pair it took 8.46 s under
 * `--coverage` and 4.32 s without, against 0.5 ms for `Buffer.equals` on the
 * same pair and tens of milliseconds for the encode + decode round trip the
 * test actually exists to exercise. `Buffer.equals` is a native memcmp, so the
 * comparison stops being the subject.
 *
