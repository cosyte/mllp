...
/**
 * Create a connected InMemory server+client pair and return the received ACKs.
 *
 * Returns `{ serverConn, clientConn, serverTransport, clientTransport, acksReceived }`.
 * `acksReceived` is populated with decoded ACK payloads (MLLP framing stripped).
 */
function createInMemoryServerClientPair(opts: Parameters<typeof createServer>[0] = {}): {
  serverConn: Connection;
  clientConn: Connection;
  serverTransport: InMemoryTransport;
  clientTransport: InMemoryTransport;
  acksReceived: Buffer[];
  errorsOnServerConn: unknown[];
} {
  const [serverTransport, clientTransport] = InMemoryTransport.pair();

  const acksReceived: Buffer[] = [];
  const errorsOnServerConn: unknown[] = [];

  // Server-side connection — created the same way _onSocketAccepted does
  const serverConn = new Connection({ transport: serverTransport });

  // Client-side connection — will receive ACKs in its 'message' event
  const clientConn = new Connection({
    transport: clientTransport,
    onMessage: (payload) => { acksReceived.push(payload); },
  });

  // Simulate TCP connect on both ends
  serverConn.notifyConnect('127.0.0.1', 2575);
  clientConn.notifyConnect('127.0.0.1', 2575);

  return { serverConn, clientTransport, clientConn, serverTransport, acksReceived, errorsOnServerConn };
}

// ---------------------------------------------------------------------------
// Helper: simulate what _onSocketAccepted does with a Connection + message handler
// ---------------------------------------------------------------------------

/**
 * Wire an auto-ACK message handler onto a server-side Connection.
 * This simulates what MllpServer._onSocketAccepted does.
 *
 * We test the observable behavior: does the client receive a correctly-formed ACK?
 * The private `_buildAutoAck` method is exercised indirectly.
 */
...
describe('SERVER-04: auto-ACK — AA mode via MllpServer over InMemoryTransport', () => {
  ...
  it('autoAck: AA — ACK MSH swaps sendingApp/receivingApp', async () => {
    ...
    const mshFields = ackStr.split('\r')[0]!.split('|');
...
describe('SERVER-04: auto-ACK — function mode', () => {
  it('autoAck: fn — fn receives (payload, meta, conn) and its return is sent as ACK', async () => {
    ...
    expect(fnCalls[0]!.payloadLen).toBeGreaterThan(0);
    expect(fnCalls[0]!.metaConnId).toBeTruthy();
...
describe('D-04 + backpressure: conn.send() returns false → MllpConnectionError emitted', () => {
   ...
   */
  it('when conn.send() returns false (backpressure), MllpConnectionError({ phase: "send" }) emitted', async () => {
    const [serverTransport, clientTransport] = InMemoryTransport.pair();
    ...
    serverConn.on('message', async (event: { payload: Buffer; connectionId: string }) => {
      const { payload } = event;
      ...
      if (!sent) {
        ...
        serverConn.emit('error', Object.freeze({
...
describe('SERVER-04: _buildAutoAck behavior via reflection (unit tests)', () => {
  it('_buildAutoAck with well-formed MSH returns Buffer containing MSA|AA|MSG001', () => {
    ...
    const buildAutoAck = serverAny['_buildAutoAck']!.bind(server);
  ...
  it('_buildAutoAck with well-formed MSH swaps sendingApp → receivingApp in ACK MSH', () => {
    ...
    const mshLine = ackStr.split('\r')[0]!;
