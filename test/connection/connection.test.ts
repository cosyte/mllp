...
function makeMockTransport() {
  ...
  const close: Mock<() => void> = vi.fn<() => void>();
  const destroy: Mock<(reason?: Error) => void> = vi.fn<(reason?: Error) => void>();
...
describe("Connection", () => {
  ...
  describe("LIFE-01/02: stateChange event", () => {
    it("emits stateChange when transitioning CONNECTING → CONNECTED", () => {
      ...
      conn.on("stateChange", (e: StateChangeEvent) => changes.push({ from: e.from, to: e.to }));
      conn.notifyConnect("127.0.0.1", 2575);
    ...
    it("stateChange includes reason when provided (CONNECTED peer drop)", () => {
      ...
      let evt: StateChangeEvent | undefined;
      c2.on("stateChange", (e: StateChangeEvent) => {
        evt = e;
      });
      m2.emit.close();
    ...
    it("stateChange reason is omitted when not provided (CONNECTING → CONNECTED)", () => {
      let payload: StateChangeEvent | undefined;
  ...
  describe("LIFE-03: lifecycle events", () => {
    ...
    it("message event contains connectionId matching connection.connectionId", () => {
      ...
      conn.on("message", (e: { connectionId: string }) => {
        received = e;
      });
      mock.emit.data(Buffer.from([0x0b, 0x41, 0x1c, 0x0d]));
  ...
  describe("destroy()", () => {
    ...
    it("calls transport.destroy()", () => {
      conn.destroy();
      expect(mock.mocks.destroy).toHaveBeenCalled();
    });
  ...
  describe("close()", () => {
    ...
    it("calls transport.close() after beforeClose resolves", async () => {
      conn.notifyConnect(null, null);
      await conn.close();
      expect(mock.mocks.close).toHaveBeenCalled();
    });
  ...
  describe("send()", () => {
    ...
    it("returns transport.write() result when writable", () => {
      ...
      expect(mock.mocks.write).toHaveBeenCalledWith(Buffer.from([0x41]));
    });
  });
