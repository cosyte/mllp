...
describe("MllpClient.send (FIFO mode, PLAN-02)", () => {
  ...
  it("Test 6: default ackTimeoutMs is 30_000 when not configured", () => {
    const { client } = buildClientOverPair();
    const ackTimeoutMs = (client as unknown as { _ackTimeoutMs: number })._ackTimeoutMs;
  ...
  it('Test 7: "ack" event fires on every match with frozen { payload, controlId, latencyMs }', async () => {
    ...
    client.on("ack", (e: { payload: Buffer; controlId: string | null; latencyMs: number }) => {
      events.push(e);
  ...
  it("Test 9: in-flight count visible via correlator stats", () => {
    ...
    const correlator = (client as unknown as { _correlator: { getStats: () => { size: number } } })
      ._correlator;
  ...
  it('Test 10: Object.freeze applied to "ack" event payload (mutation throws in strict mode)', async () => {
    ...
    const e = captured[0] as { payload: Buffer; controlId: string | null; latencyMs: number };
    expect(Object.isFrozen(e)).toBe(true);
    expect(() => {
      e.latencyMs = 99_999;
    }).toThrow();
  ...
  it("Test 11: AbortSignal pre-aborted on send() rejects synchronously", async () => {
    ...
    const ac = new AbortController();
    ac.abort();
    await expect(client.send(Buffer.from("x"), { signal: ac.signal })).rejects.toMatchObject({
      name: "AbortError",
    });
    await client.close();
  });
