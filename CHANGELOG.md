...
- 62375a9: A server can now supply the ephemeral Diffie-Hellman parameters its own site policy mandates, instead of taking whatever group the TLS library picks.
...
- a3a2eee: `0.1.0` is the first release of `@cosyte/mllp` whose public API we ask you to build on.
...
- ff5b876: Docs: the README's usage example is now one complete program the test suite runs, and the quickstart's first example is held to a committed synthetic fixture.
...
- fd04f57: The `@cosyte/hl7` dev/test peer now installs from the npm registry instead of a
  ...
  they can prove.
- 9c339c1: The Quickstart page now opens with the same program as the README: a server and a client on loopback, one message sent and its acknowledgement read.
